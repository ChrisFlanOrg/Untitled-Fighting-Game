...
class ConnectionManager {
  ...
  static addMessageHandler(type, handlerFn) {
    if (this.messageHandlers[type] == null) {
      this.messageHandlers[type] = [];
    }
    this.messageHandlers[type].push(handlerFn);
  }
  static connectHandlers = [];
  static addConnectHandler(handlerFn) {
    this.connectHandlers.push(handlerFn);
  }
  static disconnectHandlers = [];
  static addDisconnectHandler(handlerFn) {
...
// src/GameManager.ts
var localStorageItemName = "accelerometer-test-player-info";
var GameState;
((GameState2) => {
  GameState2["Setup"] = "setup";
  GameState2["Rejoin"] = "rejoin";
  GameState2["Loading"] = "loading";
  GameState2["Lobby"] = "lobby";
  GameState2["Play"] = "play";
})(GameState ||= {});
class GameManager {
  static state = "setup" /* Setup */;
  static playerId;
  static playerName;
  ...
  static {
    ConnectionManager.addConnectHandler(() => {
      ConnectionManager.send("get-game-state" /* GetGameState */);
    });
    ConnectionManager.addDisconnectHandler(() => {
      this.rejoinOrSetup();
    });
    ConnectionManager.addMessageHandler("who-are-you" /* WhoAreYou */, () => {
      ConnectionManager.send("who-i-am" /* WhoIAm */, {
        player_id: this.playerId,
        player_name: this.playerName
      });
    });
    ConnectionManager.addMessageHandler("game-state" /* GameState */, (data) => {
      const { state } = data;
  ...
  static setState(newState) {
    ...
    this.state = newState;
    import_jquery.default(`#${this.state}_page`).show();
    Object.values(GameState).forEach((key) => {
      if (key != this.state) {
        import_jquery.default(`#${key}_page`).hide();
      }
    });
    if (this.state === "lobby" /* Lobby */) {
      let touching = true;
          ...
          if (Date.now() - waitTime > 75) {
            if (touching && len > 5) {
              ...
              if (samples.length >= numSamples) {
                ...
                samples = [];
                waitTime = Date.now();
                ConnectionManager.send("phone-motion" /* PhoneMotion */, {
                  acc: {
                    x: -avg.x / avgLen,
                    y: avg.y / avgLen
                  }
  ...
  static generateNewPlayerInfo() {
    const playerInfo = {
      playerId: v4_default(),
      playerName: import_jquery.default("#player_name").val().toString()
    };
  ...
  static rejoinOrSetup() {
    ...
    if (playerInfo) {
      import_jquery.default("#rejoin").val(`Rejoin as ${playerInfo.playerName}`);
      this.setState("rejoin" /* Rejoin */);
    } else {
      this.setState("setup" /* Setup */);
    }
  }
  static setup() {
    import_jquery.default("#player_name_submit").click(() => {
      this.generateNewPlayerInfo();
      this.setState("loading" /* Loading */);
      ConnectionManager.connect();
    });
    import_jquery.default("#rejoin").click(() => {
      this.setState("loading" /* Loading */);
      ConnectionManager.connect();
    });
    import_jquery.default("#join_as_new_player").click(() => {
