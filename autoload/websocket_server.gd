...
class Peer:
	var ws: WebSocketPeer
	var last_ping_send_time: int
	var last_receive_time: int

	func _init(c_ws: WebSocketPeer):
		ws = c_ws
		last_ping_send_time = Time.get_ticks_msec()
		last_receive_time = Time.get_ticks_msec()

	func should_send_ping():
		var now = Time.get_ticks_msec()
		var ping_is_late = now - last_receive_time > (PING_TIMEOUT_SECS * 1000.0) / 2
		var ready_to_send_again = now - last_ping_send_time > (PING_RETRY_TIME_SECS * 1000.0)
		if (ping_is_late and ready_to_send_again):
	...
	func timed_out():
		var now = Time.get_ticks_msec()
		var ping_is_too_late = now - last_receive_time > (PING_TIMEOUT_SECS * 1000.0)
		return ping_is_too_late
	...
	if peer_id <= 0:
		...
		for id in peers:
			if id == -peer_id:
				continue
			if type == TYPE_STRING:
				peers[id].ws.send_text(message)
			else:
				peers[id].ws.put_packet(message)
		return OK

	assert(peers.has(peer_id))
	var socket = peers[peer_id].ws
	if type == TYPE_STRING:
		return socket.send_text(message)
	return socket.send(var_to_bytes(message))


func peer_disconnect(peer_id):
	var socket = peers[peer_id].ws
	socket.close()
...
func has_message(peer_id) -> bool:
	assert(peers.has(peer_id))
	return peers[peer_id].ws.get_available_packet_count() > 0
...
func poll() -> void:
	...
	for id in peers:
		var p: Peer = peers[id]
		p.ws.poll()
		if p.ws.get_ready_state() != WebSocketPeer.STATE_OPEN:
			client_disconnected.emit(id)
			to_remove.append(id)
			continue
		if p.ws.get_available_packet_count():
			p.last_receive_time = Time.get_ticks_msec()
		while p.ws.get_available_packet_count():
			message_received.emit(id, JSON.parse_string(get_message(id)))
	for id in peers:
		var p: Peer = peers[id]
		if (p.timed_out()):
			peer_disconnect(id)
		elif (p.should_send_ping()):
			send(id, JSON.stringify({ "type": "ping" }))
	for r in to_remove:
		peers.erase(r)
	to_remove.clear()
...
func _connect_pending(p: PendingPeer) -> bool:
	if p.ws != null:
		...
		if state == WebSocketPeer.STATE_OPEN:
			...
			peers[id] = Peer.new(p.ws)
			print("New Peer %s" % str(id))
